...
MiniAnalyzer::MiniAnalyzer(const edm::ParameterSet& pset):
  ...
  pathNames_lowIsoUnprER   = pset.getParameter<std::string>("pathNames_lowIsoUnprER");
  pathNames_lowIsoTkUnpr   = pset.getParameter<std::string>("pathNames_lowIsoTkUnpr");
  pathNames_lowIsoTkUnprER = pset.getParameter<std::string>("pathNames_lowIsoTkUnprER");
...
void MiniAnalyzer::fill_muons_variables(edm::Handle<pat::MuonCollection> muons, const math::XYZPoint &thebs, reco::Vertex PV, const RoccoR local_rc,  const RoccoR crab_rc){
  ...
  for (pat::Muon mu : *muons) {
    ...
    //did the muon fire the trigger?
    const pat::TriggerObjectStandAloneCollection muHLTMatches_iso     =  mu.triggerObjectMatchesByPath(pathNames_lowIsoUnpr);
    const pat::TriggerObjectStandAloneCollection muHLTMatches_isoEr   =  mu.triggerObjectMatchesByPath(pathNames_lowIsoUnprER);
    const pat::TriggerObjectStandAloneCollection muHLTMatches_isoTk   =  mu.triggerObjectMatchesByPath(pathNames_lowIsoTkUnpr);
    const pat::TriggerObjectStandAloneCollection muHLTMatches_isoTkEr =  mu.triggerObjectMatchesByPath(pathNames_lowIsoTkUnprER);
    ...
    bool passIso = false, passIsoEr = false, passIsoTk = false, passIsoTkEr = false,  pass = false, passTk = false;
    if(muHLTMatches_iso.size() > 0)     passIso = true;
    if(muHLTMatches_isoEr.size() > 0)   passIsoEr = true;
    if(muHLTMatches_isoTk.size() > 0)   passIsoTk = true;
    if(muHLTMatches_isoTkEr.size() > 0) passIsoTkEr = true;
    ...
    if(debug){
      ...
      std::cout << "Did it triggered " << pathNames_lowIsoUnprER <<"? " << passIsoEr << std::endl; 
      std::cout << "Did it triggered " << pathNames_lowIsoTkUnpr <<"? " << passIsoTk << std::endl; 
      std::cout << "Did it triggered " << pathNames_lowIsoTkUnprER <<"? " << passIsoTkEr << std::endl; 
      }
    
    Mu_hasTriggeredIso.push_back(passIso);
    Mu_hasTriggeredIsoEr.push_back(passIsoEr);
    Mu_hasTriggeredIsoTk.push_back(passIsoTk);
    Mu_hasTriggeredIsoTkEr.push_back(passIsoTkEr);
...
MiniAnalyzer::beginJob(){
  ...
  if(MCtruth){
    ...
    // const std::vector< float > Run_2016G = {
    //   3525.68,     316504,      1.18552e+06, 2.08941e+06, 2.92898e+06,
    //   3.96565e+06, 6.77502e+06, 2.13771e+07, 3.99399e+07, 7.5342e+07,  
    //   1.55588e+08, 2.83174e+08, 4.49937e+08, 6.2323e+08,  8.10258e+08, 
    //   1.02112e+09, 1.20339e+09, 1.32626e+09, 1.4008e+09,  1.43893e+09, 
    //   1.44394e+09, 1.42076e+09, 1.37481e+09, 1.30984e+09, 1.22781e+09,
    
    std::vector< float > Run_2016_Down = {
      247411, 1.06928e+06, 2.42828e+06, 3.56699e+06, 4.99183e+06, 6.59314e+06, 8.0973e+06, 1.99632e+07, 5.19152e+07, 1.19781e+08, 2.72775e+08, 5.1317e+08, 8.02334e+08, 1.11876e+09, 1.46299e+09, 1.78022e+09, 2.00519e+09, 2.13507e+09, 2.19849e+09, 2.23396e+09, 2.25256e+09, 2.22971e+09, 2.15421e+09, 2.04168e+09, 1.90561e+09, 1.75108e+09, 1.5846e+09, 1.4136e+09, 1.24253e+09, 1.07364e+09, 9.09771e+08, 7.55103e+08, 6.13889e+08, 4.89125e+08, 3.81982e+08, 2.92057e+08, 2.18012e+08, 1.5822e+08, 1.11102e+08, 7.51364e+07, 4.87443e+07, 3.02391e+07, 1.78961e+07, 1.00868e+07, 5.40838e+06, 2.75726e+06, 1.3369e+06, 617523, 273008, 116884, 49835.1, 22455.6, 11739.7, 7637.14, 6017.98, 5280.66, 4837.35, 4483.82, 4153.85, 3828.86, 3506.03, 3187.48, 2876.62, 2576.85, 2291.18, 2022.03, 1771.24, 1540.02, 1329.04, 1138.45, 967.946, 816.871, 684.26, 568.925, 469.521};
    
    
    std::vector< float > mix_2016_25ns_SpringMC_PUScenarioV1_PoissonOOTPU = {
    ...
    std::vector< float > mix_2016_25ns_Moriond17 = {
    ...
    LumiWeights_ = edm::LumiReWeighting(mix_2016_25ns_Moriond17, Run_2016);
    LumiWeightsUp_ = edm::LumiReWeighting(mix_2016_25ns_Moriond17, Run_2016_Up);
    LumiWeightsDown_ = edm::LumiReWeighting(mix_2016_25ns_Moriond17, Run_2016_Down);
    
       
    if(!crab){
      data = new TH1F("data", "data", Run_2016.size(), 0., Run_2016.size());
      mc = new TH1F("mc", "mc", mix_2016_25ns_Moriond17.size(), 0., mix_2016_25ns_Moriond17.size());
      
      for(unsigned int ibin = 1; ibin < Run_2016.size()+1; ++ibin){
	data->SetBinContent(ibin, Run_2016[ibin-1]);
	mc->SetBinContent(ibin, mix_2016_25ns_Moriond17[ibin-1]);
      }
      ...
      mc->SetLineWidth(2);
      mc->Draw("same");
      leg->AddEntry(data,"Data2016 All Runs");
      leg->AddEntry(mc,"MC PU_Moriond17");
      leg->Draw();
  ...
  if (youwantNTuples){  
    ...
    tree_->Branch("Mu_hasTriggeredIsoEr",&Mu_hasTriggeredIsoEr);	
  ...
  inline void MiniAnalyzer::clear_Arrays(){
    ...
    if(youwantNTuples){  
      ...
      Mu_hasTriggeredIsoEr.clear();
      Mu_hasTriggeredIsoTk.clear();
      Mu_hasTriggeredIsoTkEr.clear();
